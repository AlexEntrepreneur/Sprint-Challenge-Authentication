...
- Sessions are a way for servers to maintain state of which clients are authenticated. HTTP is stateless and so session are a way for servers to associate a request to any other request.

1. What does bcrypt do to help us store passwords in a secure manner.
- Bcrypt is a multi-language framework that implements hashing and salting using different hashing algorithms such as SHA-1, SHA-256, and MD5. It makes it easier to increase the entropy of password

1. What does bcrypt do to slow down attackers?
- Bcrypt generates a different random secret (salt) per password stored. It then concatenates this salt with the password and hashes it; repeating this process recursively n times. This makes generating rainbow tables to brute force passwords infeasible, as a new table would have to be generated per user.
